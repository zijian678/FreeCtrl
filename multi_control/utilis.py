...
def load_model(device,model_name):
    ...
    """
    # # Load Trained Model
    # model = torch.load(args.path).to(device)  ## prefix parameters only
...
class GPT2Wrapper():
    ...
    def set_value_activations2(self,
                              values_per_layer1: Dict[int, List[int]],
                              coef_value1: int,
                              values_per_layer2: Dict[int, List[int]],
                              coef_value2: int):
        """
        Uses PyTorch hooks to set the activations of each value in values_per_layer1 and values_per_layer2
        to coef_value1 and coef_value2, respectively. Only works on GPT2 from HF.

        :params values_per_layer1: dictionary with keys that correspond to layers and values that correspond to indices for the first set
        :params coef_value1: number to set the first set of values' activations to
        :params values_per_layer2: dictionary with keys that correspond to layers and values that correspond to indices for the second set
        :params coef_value2: number to set the second set of values' activations to
        """

        def value_activation_replacement_hook(values1, coef_val1, values2, coef_val2):
            # print('coef_val1:',coef_val1)
            # print('coef_val2:',coef_val2)
            def hook(module, input, output):
                output[:, :, values1] = coef_val1
                output[:, :, values2] = coef_val2
        ...
        for layer in range(self._model.config.n_layer):
            if layer in values_per_layer1:
                values1 = values_per_layer1[layer]
            else:
                values1 = []

            if layer in values_per_layer2:
                values2 = values_per_layer2[layer]
            else:
                values2 = []

            hook = self._model.transformer.h[layer].mlp.c_fc.register_forward_hook(
                value_activation_replacement_hook(values1, coef_value1, values2, coef_value2)
            )
